...
async function findData(data) {
  ...
  try {
    ...
    const datePattern = /(\d{4})년 (\d{1,2})월 (\d{1,2})일 (\d{2}):(\d{2}):(\d{2})/;
    const matches = data.submissionTime.match(datePattern);

    let year = matches[1];
    let month = matches[2].padStart(2, '0'); // 월을 2자리로
    let day = matches[3].padStart(2, '0'); // 일을 2자리로
    let hour = matches[4];
    let minute = matches[5];
    let second = matches[6];
    // // yyyy-mm-dd hh:mm:ss 날짜 형식으로 결합
    let formattedDateString = `${year}-${month}-${day} ${hour}:${minute}:${second}`;
    const currentDateTime = new Date();
    const offset = 9 * 60 * 60 * 1000; // UTC+9 시간 오프셋
    const kstDate = new Date(currentDateTime.getTime() + offset);
...
function getDateString(date) {
  ...
  return `${year}년123 ${month}월 ${day}일 ${hours}:${minutes}:${seconds}`;
}
...
async function makeDetailMessageAndReadme(data) {
  ...
  // const directory = await getDirNameByOrgOption(`백준/${level.replace(/ .*/, '')}/${problemId}. ${convertSingleCharToDoubleChar(title)}`, langVersionRemove(language, null));
...
function parseProblemDescription(doc = document) {
  ...
  if (problemId && problem_description) {
    ...
    return { problemId, problemDescription: problem_description, problemInput: problem_input, problemOutput: problem_output };
  }
  return {};
}
