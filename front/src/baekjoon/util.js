...
//     fixedComponent = document.createElement('div');
//     fixedComponent.classList.add('fixed-component');
//     fixedComponent.innerHTML = `<div>틀렸습니다.틀렸습니다 .테스트!!!</div>`;
//     elem.appendChild(fixedComponent);
//   } else if (elem !== null && fixedComponent) {
//     fixedComponent.innerHTML = `<div>컴파일 에러, 테스트!!!!!!!</div>`;
//   }
//   setTimeout(() => {
//     if (fixedComponent) {
//       elem.removeChild(fixedComponent);
...
//     elem.appendChild(fixedComponent);
//   } else if (elem !== null && fixedComponent) {
//     fixedComponent.innerHTML = `<div>메모리 초과</div>`;
//   }
...
//     elem.appendChild(fixedComponent);
//   } else if (elem !== null && fixedComponent) {
//     fixedComponent.innerHTML = `<div>출력 초과</div>`;
//   }
...
//   /* eslint-disable */
//   return hasNotSubtask(a.result, b.result) ? (a.runtime === b.runtime ? (a.memory === b.memory ? (a.codeLength === b.codeLength ? -(a.submissionId - b.submissionId) : a.codeLength - b.codeLength) : a.memory - b.memory) : a.runtime - b.runtime) : compareResult(a.result, b.result);
//   /* eslint-enable */
//   // prettier-ignore-end
// }
...
function failedAni() {
  ...
  chrome.runtime.sendMessage({ action: 'getShowCharacter' }, (response) => {
    if(response.showCharacter === true){
      const event = new CustomEvent('playAnimation', {
        detail: { animation: 'Death', duration: 2000 }
      });
      document.dispatchEvent(event);
...
function errorTimeLimitAni() {
  ...
  chrome.runtime.sendMessage({ action: 'getShowCharacter' }, (response) => {
    if(response.showCharacter === true){
      ...
      const event = new CustomEvent('playAnimation', {
        detail: { animation: 'Swim', duration: 5000 }
      });
      document.dispatchEvent(event);
...
function errorCompilationAni() {
  ...
  // Confetti 효과를 위한 CustomEvent 발생
  document.dispatchEvent(new CustomEvent('compileError'));
  // 캐릭터 애니메이션 변경
  chrome.runtime.sendMessage({ action: 'getShowCharacter' }, (response) => {
    if(response.showCharacter === true){
      const event = new CustomEvent('playAnimation', {
        detail: { animation: 'Fly', duration: 5000 }
      });
      document.dispatchEvent(event);
...
function errorMemoryLimitAni() {
  ...
  chrome.runtime.sendMessage({ action: 'getShowCharacter' }, (response) => {
    if(response.showCharacter === true){
      ...
      const event = new CustomEvent('playAnimation', {
        detail: { animation: 'Fear', duration: 5000 }
      });
      document.dispatchEvent(event);
